...
class Settings:
    ...
    # Below this Tesseract word-confidence (0-100), a short Latin-script
    # token found inside an Arabic-majority OCR line is treated as likely
    # noise even if it happens to match a real English dictionary word —
    # see services/ocr_service.py's _clean_ocr_artifacts. Only applied when
    # confidence data was actually collected (the tiered OCR fast path);
    # cleanup degrades to whitelist+dictionary-only otherwise.
    OCR_ARTIFACT_MIN_CONFIDENCE: float = _float("OCR_ARTIFACT_MIN_CONFIDENCE", 45.0)
