//const BASE = "/api";
const BASE = "http://localhost:8000/api";   // was "/api"
...
const UPLOAD_TIMEOUT_MS = 1_200_000; // OCR + embed can be slow
 ...
 */
async function apiFetch(path: string, init: RequestInit = {}, timeoutMs: number = REQUEST_TIMEOUT_MS): Promise<Response> {
...
export async function uploadXlsx(files: File[]): Promise<UploadResponse> {
  const form = new FormData();
  files.forEach((f) => form.append("files", f));

  const res = await apiFetch("/upload/xlsx", {
    method: "POST",
    body: form,
  }, UPLOAD_TIMEOUT_MS);
